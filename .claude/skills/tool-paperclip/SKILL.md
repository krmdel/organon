...
### pull â Download papers or files
```bash
...
7. **Supplements / data** â `paperclip ls /papers/<id>/supplements/` then process with `sed`/`awk` via bash, or `paperclip pull ...` for raw files.
