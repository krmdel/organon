...
How to replace AI patterns with human writing. Two modes: generic (no research context) and voice-matched (with research_context/research-profile.md loaded).
...
When `research_context/research-profile.md` is loaded, replacements should use the researcher's actual patterns instead of generic human phrasing.

### How to Map Voice Profile to Replacements

**1. Connectors:** Replace AI transitions with the researcher's preferred connectors.
...
**2. Vocabulary:** Replace AI buzzwords with the researcher's preferred terms.
- Read the "Vocabulary" → "Preferred" list
- Read the "Vocabulary" → "Avoid" list (treat these as additional AI tells)
- Example: If researcher avoids "leverage" and prefers "plug in" → "leverage AI tools" becomes "plug in AI tools"

**3. Rhythm:** Match the researcher's sentence patterns.
- Read "Rhythm & Structure" section
- If researcher prefers short punchy + medium mix → break AI's long sentences
- If researcher uses fragments → add fragments for pacing

**4. Energy:** Match the researcher's intensity.
- Read "Personality Traits" and "Tone Spectrum"
- If researcher is playful/energetic → replacements should be more casual
- If researcher is formal/precise → replacements stay professional but still human

**5. Signature patterns:** Apply the researcher's distinctive moves.
