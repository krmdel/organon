...
export interface PaperResult {
  ...
  doi: string | null;
  url: string;
  source: "pubmed" | "arxiv" | "openalex" | "semanticscholar" | "paperclip";
...
function parsePubmedArticle(articleXml: string): PaperResult | null {
  ...
  return {
    // Phase 3 (fix-sprint): bare PMID. The "source" field already disambiguates;
    // adding a `pmid:` prefix here cascades into `source_ids.pmid = "pmid:..."`
    // and `id = "pmid-pmid:..."` (FIXPLAN Finding #4).
    id: pmid,
    title,
    authors,
    abstract,
