...
function mapWorkToResult(work: OpenAlexWork): PaperResult {
  ...
  // Phase 3 (fix-sprint): OpenAlex returns the W-id as a full URL
  // (https://openalex.org/W4400000000). We need the bare W-id for the
  // PaperArtifact id — the URL form has slashes that break the
  // `papers/<id>.json` filename and silently corrupt the persist path
  // (FIXPLAN Finding #5: only PubMed sources persisted). The original URL
  // is still emitted as `url` so the UI deep-link works.
  const bareWorkId = work.id.replace(/^https?:\/\/openalex\.org\//i, "");
  return {
    id: bareWorkId,
